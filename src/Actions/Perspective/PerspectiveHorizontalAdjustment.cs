namespace Loupedeck.LightroomPlugin
{
    ...
    public class PerspectiveHorizontalAdjustment : ActionEditorAdjustment
    {
        private const String StepControlName = "StepValue";

        public PerspectiveHorizontalAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Perspective";
            this.Name = "PerspectiveHorizontalAdjustment";
            this.DisplayName = "Perspective: Horizontal";
            this.Description = "Adjust perspective horizontal with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Perspective: Horizontal (Step: {stepValue:F2})");
                }
            }
        }
        ...
        private void SendRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("PerspectiveHorizontal", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("PerspectiveHorizontal", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending perspective horizontal {action} request: {ex.Message}");
                }
            });
        }
    }
