namespace Loupedeck.LightroomPlugin
{
    ...
    public class ClarityAdjustment : ActionEditorAdjustment
    {
        private const String StepControlName = "StepValue";

        public ClarityAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Effects";
            this.Name = "ClarityAdjustment";
            this.DisplayName = "Effects: Clarity";
            this.Description = "Adjust clarity with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Effects: Clarity (Step: {stepValue:F2})");
                }
            }
        }
        ...
        protected override Boolean ApplyAdjustment(ActionEditorActionParameters actionParameters, Int32 diff)
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    SendClarityRequest("increment", stepValue);
                }
            }
            else if (diff < 0)
            {
                ...
                {
                    SendClarityRequest("decrement", stepValue);
                }
            }
            return true;
        }

        private void SendClarityRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("Clarity2012", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("Clarity2012", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending clarity {action} request: {ex.Message}");
                }
            });
        }
    }
