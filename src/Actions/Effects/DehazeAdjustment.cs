namespace Loupedeck.LightroomPlugin
{
    ...
    public class DehazeAdjustment : ActionEditorAdjustment
    {
        private const String StepControlName = "StepValue";

        public DehazeAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Effects";
            this.Name = "DehazeAdjustment";
            this.DisplayName = "Effects: Dehaze";
            this.Description = "Adjust dehaze with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Effects: Dehaze (Step: {stepValue:F2})");
                }
            }
        }
        ...
        protected override Boolean ApplyAdjustment(ActionEditorActionParameters actionParameters, Int32 diff)
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    SendDehazeRequest("increment", stepValue);
                }
            }
            else if (diff < 0)
            {
                ...
                {
                    SendDehazeRequest("decrement", stepValue);
                }
            }
            return true;
        }

        private void SendDehazeRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("Dehaze", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("Dehaze", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending dehaze {action} request: {ex.Message}");
                }
            });
        }
    }
