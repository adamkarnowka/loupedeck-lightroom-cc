namespace Loupedeck.LightroomPlugin
{
    ...
    public class WhitesAdjustment : ActionEditorAdjustment
    {
        private const String StepControlName = "StepValue";

        public WhitesAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Light";
            this.Name = "WhitesAdjustment";
            this.DisplayName = "Light: Whites";
            this.Description = "Adjust whites with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Light: Whites (Step: {stepValue:F2})");
                }
            }
        }
        ...
        protected override Boolean ApplyAdjustment(ActionEditorActionParameters actionParameters, Int32 diff)
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    SendWhitesRequest("increment", stepValue);
                }
            }
            else if (diff < 0)
            {
                ...
                {
                    SendWhitesRequest("decrement", stepValue);
                }
            }
        ...
        private void SendWhitesRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("Whites2012", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("Whites2012", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending whites {action} request: {ex.Message}");
                }
            });
        }
    }
