namespace Loupedeck.LightroomPlugin
{
    ...
    public class ExposureAdjustment : ActionEditorAdjustment
    {
        ...
        public ExposureAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Light";
            this.Name = "ExposureAdjustment";
            this.DisplayName = "Light: Exposure";
            this.Description = "Adjust exposure with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Light: Exposure (Step: {stepValue:F2})");
                }
            }
        }
        ...
        protected override Boolean ApplyAdjustment(ActionEditorActionParameters actionParameters, Int32 diff)
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    SendExposureRequest("increment", stepValue);
                }
            }
            else if (diff < 0)
            {
                ...
                {
                    SendExposureRequest("decrement", stepValue);
                }
            }
        ...
        private void SendExposureRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("Exposure2012", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("Exposure2012", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending exposure {action} request: {ex.Message}");
                }
            });
        }
    }
