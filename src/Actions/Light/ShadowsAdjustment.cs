namespace Loupedeck.LightroomPlugin
{
    ...
    public class ShadowsAdjustment : ActionEditorAdjustment
    {
        private const String StepControlName = "StepValue";

        public ShadowsAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Light";
            this.Name = "ShadowsAdjustment";
            this.DisplayName = "Light: Shadows";
            this.Description = "Adjust shadows with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Light: Shadows (Step: {stepValue:F2})");
                }
            }
        }
        ...
        protected override Boolean ApplyAdjustment(ActionEditorActionParameters actionParameters, Int32 diff)
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    SendShadowsRequest("increment", stepValue);
                }
            }
            else if (diff < 0)
            {
                ...
                {
                    SendShadowsRequest("decrement", stepValue);
                }
            }
        ...
        private void SendShadowsRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("Shadows2012", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("Shadows2012", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending shadows {action} request: {ex.Message}");
                }
            });
        }
    }
