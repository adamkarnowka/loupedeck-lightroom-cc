namespace Loupedeck.LightroomPlugin
{
    ...
    public class VibranceAdjustment : ActionEditorAdjustment
    {
        private const String StepControlName = "StepValue";

        public VibranceAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Tint";
            this.Name = "VibranceAdjustment";
            this.DisplayName = "Tint: Vibrance";
            this.Description = "Adjust vibrance with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Tint: Vibrance (Step: {stepValue:F2})");
                }
            }
        }
        ...
        protected override Boolean ApplyAdjustment(ActionEditorActionParameters actionParameters, Int32 diff)
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    SendVibranceRequest("increment", stepValue);
                }
            }
            else if (diff < 0)
            {
                ...
                {
                    SendVibranceRequest("decrement", stepValue);
                }
            }
        ...
        private void SendVibranceRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("Vibrance", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("Vibrance", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending vibrance {action} request: {ex.Message}");
                }
            });
        }
    }
