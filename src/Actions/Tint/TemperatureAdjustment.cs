namespace Loupedeck.LightroomPlugin
{
    ...
    public class TemperatureAdjustment : ActionEditorAdjustment
    {
        ...
        public TemperatureAdjustment() : base(hasReset: false)
        {
            this.GroupName = "Tint";
            this.Name = "TemperatureAdjustment";
            this.DisplayName = "Tint: Temperature";
            this.Description = "Adjust temperature with knob rotation";
        ...
        private void OnControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            if (e.ControlName.EqualsNoCase(StepControlName))
            {
                ...
                {
                    e.ActionEditorState.SetDisplayName($"Tint: Temperature (Step: {stepValue:F2})");
                }
            }
        }
        ...
        protected override Boolean ApplyAdjustment(ActionEditorActionParameters actionParameters, Int32 diff)
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    SendTemperatureRequest("increment", stepValue);
                }
            }
            else if (diff < 0)
            {
                ...
                {
                    SendTemperatureRequest("decrement", stepValue);
                }
            }
        ...
        private void SendTemperatureRequest(String action, Double stepValue)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (action == "increment")
                    {
                        await LightroomPlugin.WebSocketClient.IncrementParameterAsync("Temperature", stepValue);
                    }
                    else if (action == "decrement")
                    {
                        await LightroomPlugin.WebSocketClient.DecrementParameterAsync("Temperature", stepValue);
                    }
                }
                catch (Exception ex)
                {
                    PluginLog.Error($"Error sending temperature {action} request: {ex.Message}");
                }
            });
        }
    }
